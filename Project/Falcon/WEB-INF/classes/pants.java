...
public class pants extends HttpServlet {

	private HashMap<String, Product> items;
	private HashMap<String, Product> pants;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//pw.println("<input class = 'submit-button' type = 'submit' name = 'MySQLDataStoreUtilities.sortPageDetails();' value = 'Price Low to high'>");

		//pw.println("<button onclick='myFunction()'>Try it</button>");
		
		
		
		for (HashMap.Entry<String, Product> entry : pMap.entrySet()) {
	Product product = entry.getValue();
	if(product.getDisplay_under().equalsIgnoreCase("pant")){
		pw.println("<tr>");
		pw.println("<td>");
